...
Type: user-confirmation
Decision: 后端采用「零氪 serverless 栈」（参考用户提供的 cali.so 零成本建站文档）：部署 Vercel（免费额度）；运行时 Next.js Route Handlers（与前端同 app/，Serverless Functions），契合 DEC-014/DEC-011 轻后端；持久化 Neon PostgreSQL（serverless）+ Drizzle ORM（schema/迁移 drizzle-kit，db:push）；缓存/计数/限流 Upstash Redis（平台聚合统计 FR-140、抗滥用限流 NFR-006）；邮件 Resend（通知，站内优先 ASM-048、邮件通道按需启用）；鉴权 GitHub OAuth 经 Auth.js（NextAuth v5），GitHub 为规范身份（DEC-006）——参考文档用 Clerk（仅 GitHub 连接）亦为可接受的托管替代。
Rationale: 用户以「技术栈参考此文档」回应阶段 13 的 A 级栈问题；该栈全部免费额度、serverless、与已选 Next.js 前端同仓共类型，最贴合 DEC-011 轻后端与零成本目标。鉴权从文档的 Clerk 收敛为 Auth.js+GitHub 以最忠实 DEC-006（GitHub 即身份、无额外身份厂商），Clerk 列为等价替代。
Source: 用户于 2026-06-23 的消息「技术栈可以参考这个：/Users/zyongzhu/ThirdSpace/03-知识/开发工具/20260319_如何复刻本网站_零氪快速建博客.md」。
Affected Artifacts: PROJECT_CONTEXT.md, aies/04-backend/BACKEND_SPEC.md, MODULE_WORKFLOW_SPEC.md, SERVICE_CONTRACT.md, 阶段16 server/ 实现
Invalidates: none
Owner: user
Revisit Trigger: 若改用非 Vercel 部署、或以 Clerk 取代 Auth.js、或换 DB/ORM。
Status: active
```
...
Stage: 13~15-backend
Type: user-confirmation
Decision: 用户签字通过后端规格（13 BACKEND_SPEC / 14 MODULE_WORKFLOW_SPEC / 15 SERVICE_CONTRACT），并接受交叉审核抛出的 5 个契约决策推荐默认：(1) RISK-004 前端两套 trust/skills 路径中旧路径(API-027 /api/trust/:login、API-030 /api/skills)经扫描零组件消费=死脚手架→标 deprecated 收敛到 API-023/028，前端死 hook 待清理；(2) ASM-120 写端点（创建/接受/拒绝/撤销交换、收藏/认可/举报）契约形状认可、前端 MSW+UI 待阶段16补；(3) MSW 形状对齐以契约为准（/api/exchanges 加 topics、/api/notifications 加 unreadCount）；(4) ASM-118 本版 Agent=User 行动者角色、外部 agent 签名身份留扩展面；(5) 邮件通道 Resend 站内优先、本期不启用邮件。15b 契约完整性门通过。
Rationale: 三文档内聚、交叉审核双向孤儿扫描全清（无孤儿/越权端点、INV→TEST 全覆盖、FR 全落点），契约对齐前端实际调用；5 决策均非阻断且采推荐默认。解锁阶段 16 后端 TDD。
Source: 用户于 2026-06-23 的消息「接受」。
Affected Artifacts: aies/04-backend/{BACKEND_SPEC,MODULE_WORKFLOW_SPEC,SERVICE_CONTRACT}.md, ID_REGISTRY（API-027/030→deprecated、RISK-004→mitigated）, ASM-118/120→active, ARTIFACT_MANIFEST（13/14/15→passed）, 阶段16 server/ + 前端 MSW 对齐
Invalidates: API-027、API-030（弃用，替代 API-023/028）
Owner: user
Revisit Trigger: 若后续需要外部 agent 签名身份、或启用邮件通道、或保留旧 trust/skills 路径。
Status: active
```
...
Type: user-confirmation
Decision: 用户签字接受 Know-share 可发布基线，阶段 17 集成发布通过，product-to-code 全 17 阶段流水线完成。基线 = 完整目标产品规格（FR/NFR/FLOW/ENT/INV/IA/UI/PAGE/COMP/API/TEST）+ 前端实现(25 路由,Playwright 11/11)+ 后端实现(6 域 46 API,TDD)+ 前后端契约一致 + 跨域集成 + 不变量服务端强制；全量 480 测试通过、typecheck 0、lint 0 错误、reconcile 0 错误。接受显式剩余风险为部署配置/打磨/已确认延后项（部署凭据、FE↔真实BE 浏览器联调、next/font 构建网络、写 UI 接线、ASM-118/048 延后等），不阻断核心产品成立。
Rationale: 集成报告与发布评审显示产品范围/前端/后端/契约/测试/浏览器验证/已知风险与用户批准对齐（skill 完成标准）。
Source: 用户于 2026-06-23 的消息「OK」。
Affected Artifacts: aies/05-release/{INTEGRATION_REPORT,RELEASE_REVIEW}.md（→passed）, ARTIFACT_MANIFEST.yaml（stage 17→passed，项目完成）
Invalidates: none
Owner: user
Revisit Trigger: 配置部署 env 实跑联调、接线延后写 UI、启用邮件/外部 agent 身份，或任何范围变更（按 change-control 重开相应阶段）。
Status: active
```
