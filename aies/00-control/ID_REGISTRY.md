...
| `API-001~053` | 服务契约操作 | 后端 API 端点（会话/模块/搜索/交换/信任/技能/通知/设置-联系方式/审核/统计 + 写操作） | `04-backend/SERVICE_CONTRACT.md` | `FR-*`, `ENT-*`, `INV-03/04/09`, `DEC-017` | draft | 对齐前端 query hooks/MSW；公开读不输出 contact/原始内容。**API-027(/api/trust/:login)、API-030(/api/skills) 经 DEC-018 弃用(deprecated)**，替代 API-023/028（旧前端 hook 死脚手架待清）。 |
...
| `RISK-002~005` | 风险 | 栈选型残留/非法状态迁移/前端双路径(trust,skills)待收敛/信任重算竞态 | `04-backend/*` | `DEC-017`, `FLOW-003` | open | **RISK-004 经 DEC-018 转 mitigated**（旧路径=死脚手架，弃用 API-027/030 收敛）；RISK-002/003/005 设计期缓解、阶段16 实现守。 |
